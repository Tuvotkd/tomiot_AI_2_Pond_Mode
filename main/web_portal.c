...
"            <span><span style='color:#8b5cf6;'>■</span> System (5B)</span>"
...
"      <div class='card' style='display:flex; flex-wrap:wrap; gap:20px; padding:20px; min-height:400px; color:var(--t);'>"
...
"        </div>"
"      </div>"
"      <div class='card' style='margin-top:20px; padding:20px; color:var(--t);'>"
...
"      <div class='card' style='background:#0f172a; border-radius:16px; padding:20px; color:#f8fafc; border:1px solid #334155; display:flex; flex-direction:column; gap:15px; height:750px; box-sizing:border-box;'>"
...
"    document.getElementById('bar_sys').style.width = ((5 / total) * 100) + '%';"
"    let html = '';"
"    d.blocks.forEach(b => {"
...
"      let html = '';"
"      d.tasks.sort((a,b) => b.priority - a.priority).forEach(t => {"
...
"  fetch(\'/api/system/logs?last_seq=\' + window.lastLogSeq).then(r=>r.json()).then(d=>{"
"    if (d && d.length > 0) {"
